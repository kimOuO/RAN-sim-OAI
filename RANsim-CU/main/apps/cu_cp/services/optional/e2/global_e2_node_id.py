...
_DEFAULT_RAN_FUNC_ID_ANR = 6
_DEFAULT_ANR_OID = "1.3.6.1.4.1.53148.1.1.2.6"  # E2SM-ANR (gNB SON trigger, 自定)
...
def read_global_e2_node_id() -> dict:
    ...
    return {
        ...
        "ran_functions": [
            ...
            {
                # E2SM-ANR(gNB SON 觸發,自定)。adapter 端由 E2SM_ANR_ENABLE gate 是否廣播;
                # 預設關 → 不進 E2 Setup,不影響 KPM/RC 對 RIC 註冊。
                "ran_function_id": _DEFAULT_RAN_FUNC_ID_ANR,
                "ran_function_oid": _DEFAULT_ANR_OID,
                "ran_function_revision": 1,
                "service_model": "ANR",
                "version": "v01.00",
            },
            {
                # E2SM-DTFULLKPM(vendor)— E2_data_example.md 全格式 JSON indication。
                # adapter 端由 E2SM_FULLKPM_ENABLE gate 是否廣播(預設 on)。
                "ran_function_id": 5,
                "ran_function_oid": "1.3.6.1.4.1.53148.1.1.2.100",
                "ran_function_revision": 1,
                "service_model": "FULLKPM",
                "version": "v01.00",
            },
        ],
