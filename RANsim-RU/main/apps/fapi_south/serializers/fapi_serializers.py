...
class _PduSerializer(serializers.Serializer):
    ...
    # prb_count=0 = measurement-only PDU (CSI-RS 對齊 OAI: 沒 traffic 也要算 channel)
    prb_count = serializers.IntegerField(min_value=0, max_value=275)
    ...
    # CU-CP 透過 DU 帶下來的 serving cell。空字串 = 舊行為 fallback 到 Sionna argmax.
    cell_id = serializers.CharField(required=False, allow_blank=True, default="")
